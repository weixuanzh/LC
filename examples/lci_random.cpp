...
void threadFcn(unsigned seed)
{
  ...
  *(uint64_t*)&unused = 1;
  ...
  while (true) {
    ...
    uint64_t t1 = chrono::duration_cast<chrono::microseconds>(
                      chrono::high_resolution_clock::now().time_since_epoch())
                      .count();
    ...
    if (now - lastSend > interval) {
      ...
      while (LCI_puts(ep, unused, dest, 0, LCI_DEFAULT_COMP_REMOTE) ==
             LCI_ERR_RETRY) {
      }
      lastSend = chrono::duration_cast<chrono::microseconds>(
                     chrono::high_resolution_clock::now().time_since_epoch())
                     .count();
  ...
  while (nRecv < expectedRecv) {
    ...
    // Forced exit for debugging
    uint64_t t = chrono::duration_cast<chrono::microseconds>(
                     chrono::high_resolution_clock::now().time_since_epoch())
                     .count();
...
int main(int argc, char** args)
{
  ...
  while ((next = getopt(argc, args, "t:l:u:m:r:")) != -1) {
    switch (next) {
      ...
      case 'm':
        ...
        break;
      case 'r':
        interval = 1000000.0 / atoi(optarg);
        break;
  ...
  for (int i = 0; i < LCI_NUM_PROCESSES; i++) {
    ...
    if (i == LCI_NUM_PROCESSES - 1) {
      ...
      // Make sure the current and future random number can meet upper and lower
      // requirements
  ...
  // Message count is sent in the tag of the message (corrupted memory when
  // sending with data)
  LCI_short_t src;
  for (int i = LCI_RANK; i < LCI_RANK + LCI_NUM_PROCESSES; i++) {
    *(uint32_t*)&src = (uint32_t)1;
    while (LCI_puts(ep, src, i % LCI_NUM_PROCESSES,
                    (*sendCounts)[i % LCI_NUM_PROCESSES].data.load(),
  ...
  for (int i = 0; i < nThreads; i++) {
    ts.push_back(
        thread(threadFcn, time(NULL) + i * LCI_NUM_PROCESSES + LCI_RANK));
  }
