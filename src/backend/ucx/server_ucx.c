...
// addrs_length is necessary as an input since original address has nulls inside it
void encode_ucp_address(char* my_addrs, int addrs_length, char* encoded_value) {
    ...
    for (int i = 0; i < segs; i++) {
        sprintf(encoded_value + 2 * i * sizeof(uint64_t), "%016lx", ((uint64_t*)my_addrs)[i]);
    }
}
...
void publish_address(char* encoded_addrs, int endpoint_id, size_t* num_segments) {
    ...
    *num_segments = (length + LCM_PMI_STRING_LIMIT - 2) / (LCM_PMI_STRING_LIMIT - 1);
    ...
    for (int i = 0; i < *num_segments; i++) {
        ...
        if (i == *num_segments - 1) {
            memcpy(seg, encoded_addrs + i * (LCM_PMI_STRING_LIMIT - 1), length - i * (LCM_PMI_STRING_LIMIT - 1));
        } else {
            memcpy(seg, encoded_addrs + i * (LCM_PMI_STRING_LIMIT - 1), LCM_PMI_STRING_LIMIT - 1);
        }
...
void get_address(size_t num_segments, int endpoint_id, int rank, char* combined_addrs) {
    for (int i = 0; i < num_segments; i++) {
        ...
        memcpy(combined_addrs + i * (LCM_PMI_STRING_LIMIT - 1), seg, LCM_PMI_STRING_LIMIT - 1);
    }
}
...
{   
    ...
    *endpoint_pp = (LCIS_endpoint_t)endpoint_p;
    ...
    // Keys to use when publishing address (number of segments encoded address is divided into)
